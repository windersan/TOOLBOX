...
export class UploaderService {
  ...
  upload(file: File) {
    ...
    const req = new HttpRequest('POST', 'http://arctrade.azurewebsites.net/api/files', file, {
      reportProgress: true
    });
  ...
  getLast():  Observable<ResumeId>{
    console.log('UploaderService.getLast() called');
     var g = this.http.get<ResumeId>('http://arctrade.azurewebsites.net/api/files/');
      console.log(g);
     return g;
  }
  ...
  update(resumeId: ResumeId, userid: number):  Observable<void>{
    console.log('UploaderService.getLast() called');
     var str = 'http://arctrade.azurewebsites.net/api/applications/' + userid;
